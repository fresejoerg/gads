...
requires:
  ...
  capabilities: [statsmodels, pandas, numpy]
...
dag:
  ...
  - id: estimate_and_refute
    intent: >
      ...
      statsmodels, realized as code in this task:
      ...
      (2) ESTIMATE — fit an ordinary least squares regression of `outcome_col` on
          `treatment_col` plus ALL of `confounder_cols` using the statsmodels formula
          API (smf.ols). The ATE is the fitted coefficient of `treatment_col`. Store
          it in `ate`.
...
# ——— GLOBAL INVARIANTS ———
invariants:
  - "LINEAR-REGRESSION ADJUSTMENT IS MANDATORY: the ATE is the treatment coefficient of an OLS regression of the outcome on treatment + confounders, fitted with statsmodels. Use ONLY statsmodels + pandas + numpy. NEVER import dowhy, econml, or any causal-inference package — this recipe is a delegation-dial API-surface arm (approach_docs/014) and the library choice is the experimental variable."
...
# Causal Effect Estimation with statsmodels OLS — Directed variant (dial rung D3)
...
linear-regression adjustment IS OLS with covariates, and the refuters mirror
DoWhy's placebo_treatment_refuter and data_subset_refuter — but realized through
a HIGH-PREVALENCE API surface (statsmodels formula API) instead of the niche
DoWhy API. No skills attached, no native steps: every task rungs at D3. Holding
task, benchmarks, gold ATEs, and rung constant while varying only the required
library isolates the API-familiarity variable (hypothesis H1).

## When to use
Only for delegation-dial measurement runs, pinned explicitly by a spec
(`recipe_id: causal_effect.observational.ols.directed`). Never for production —
